...
public class LikelihoodWeighting extends StochasticPropagation {
    
    public LikelihoodWeighting(ProbNet probNet) throws NotEvaluableNetworkException.NotApplicableNetwork, NotEvaluableNetworkException.UnsatisfiedConstraints, ConstraintViolatedException {
        super(probNet);
    }
    ...
    protected double[] getValuesSampledAndWeight() throws NonProjectablePotentialException {
        ...
        for (Variable variable : variablesOfEvidence) {
            potentialsOfEvidence.add(probNet.getNode(variable).getPotential());
        }
