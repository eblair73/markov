...
package org.openmarkov.inference.algorithm.dlimidevaluation;
...
public class StrategyManager {
    ...
    private final List<Node> decisionNodes; // This holds the node
    private final int[] nStates;            // This its number of states
    private final int[] nParentConfigs;    // This its number of parent configurations
    
    private final int horizon; // The further slice index
     ...
     **/
    private int[] options;
    private final int[] maximumOptions;     // Maximum option for each element
    
    
    /**
     * Constructs the strategy manager for a given expanded temporal network and initializes
     * a random strategy. Orders nodes by time slice and builds the strategy encoding.
     *
     * @param probNet the expanded temporal probabilistic network
     * @param horizon the furthest time slice index
     */
     ...
     */
    public List<Potential> bruteForce(int limit) throws NonProjectablePotentialException {
        
        OpenMarkovLogger.LOGGER.info("");
        OpenMarkovLogger.LOGGER.info("[Beginning brute force]");
        ...
        try { // An IndexOutOfBoundsException will break this loop
            ...
            for (int i = 0; i < limit + 1; i++) { // This limits the evaluation to maximumEvaluated strategies
                ...
                if (utility > bestUtility) {
                    ...
                    OpenMarkovLogger.LOGGER.info("A new best utility was found: " + bestUtility);
                }
                nStrategies++;
                
                if (nStrategies % (limit / 20) == 0) {
                    OpenMarkovLogger.LOGGER.info((nStrategies) + " strategies evaluated");
                }
            }
        ...
        OpenMarkovLogger.LOGGER.info("Best utility of the " + nStrategies + " strategies evaluated: " + bestUtility);
        for (Potential policy : bestStrategy) {
            OpenMarkovLogger.LOGGER.info(policy.toString());
        }
        OpenMarkovLogger.LOGGER.info("Time elapsed: " + timeElapsed + " ms");
     ...
     * @return The strategy in its potential form
     */
    public List<Potential> randomWalk(int strategiesEvaluated) throws NonProjectablePotentialException {
        
        // Evaluate strategiesEvaluated random strategies ()
        OpenMarkovLogger.LOGGER.info("");
        OpenMarkovLogger.LOGGER.info("[Random walk]");
        long startTime = System.currentTimeMillis();
     ...
     */
    public double evaluate() throws NonProjectablePotentialException {
