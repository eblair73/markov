...
/**
 * Abstract base class for all inference tasks that use the variable elimination algorithm.
 * Provides common preprocessing steps (network expansion, evidence extension, discretization,
 * policy imposition) and a heuristic factory for determining variable elimination order.
 */
public abstract class VariableElimination extends InferenceAlgorithm {
	 ...
	 */
    public VariableElimination(ProbNet network) throws NotEvaluableNetworkException.NotApplicableNetwork, NotEvaluableNetworkException.UnsatisfiedConstraints, ConstraintViolatedException {
		super(network);
        
        setHeuristicFactory((probNet, variables) -> new SimpleElimination(probNet, variables));
	}
	 ...
	 */
    void exactAlgorithmsPreprocessing() throws IncompatibleEvidenceException.EvidenceIsIncompatibleWithOther, NonProjectablePotentialException {
	...
	/**
	 * Creates an elimination heuristic instance by projecting the partial order of variables
	 * and delegating to the configured heuristic factory.
	 *
	 * @param markovNetworkInference the Markov network used for inference
	 * @param queryVariables         variables of interest (not to be eliminated)
	 * @param evidenceVariables      observed variables from evidence
	 * @param conditioningVariables  variables to condition on (kept in results)
	 * @param variablesToEliminate   variables that should be eliminated
	 * @return a configured elimination heuristic
	 */
