...
public class DANOperations {
     ...
     */
    public static ProbNet instantiate(ProbNet originalDAN, Variable variable, State state) throws IncompatibleEvidenceException.EvidenceIsIncompatibleWithOther, NonProjectablePotentialException {
        ProbNet instantiatedNet = originalDAN.copy();
        
        Node originalNode = originalDAN.getNode(variable.getName());
        ...
        for (Link<Node> link : originalDAN.getLinks(originalNode)) {
            if (!link.getFrom().equals(originalNode)) {
                continue;
            }
            ...
            if (link.hasRestrictions()) {
                ...
                if (nonRestrictedStates.isEmpty()) {
                    ...
                    Deque<Node> disposableNodes = new ArrayDeque<>();
                    ...
                    } catch (NonProjectablePotentialException | NotSupportedOperationException |
                             IncompatibleEvidenceException.EvidenceIsIncompatibleWithOther e) {
        ...
        // Prune chance nodes whose probability distribution became all-zero after projection.
        // This happens when a variable is not relevant in this branch (e.g., not revealed).
        pruneZeroProbabilityChanceNodes(instantiatedNet);
        return instantiatedNet;
    }

    /**
     * Removes chance nodes with all-zero probability distributions and their dependent descendants.
     * A chance variable with P=[0,...,0] indicates it's irrelevant in this branch of the DAN
     * (e.g., a variable that is not revealed for the current decision state).
     */
    private static void pruneZeroProbabilityChanceNodes(ProbNet net) {
        boolean changed = true;
        while (changed) {
            changed = false;
            for (Node node : new ArrayList<>(net.getNodes(NodeType.CHANCE))) {
                if (hasAllZeroProbability(node)) {
                    // Remove this node and its descendants (same logic as restriction pruning)
                    Deque<Node> disposableNodes = new ArrayDeque<>();
                    disposableNodes.push(node);
                    while (!disposableNodes.isEmpty()) {
                        Node disposableNode = disposableNodes.pop();
                        if (disposableNode.getNodeType() != NodeType.DECISION
                                || !ProbNetOperations.hasPredecessorDecision(disposableNode, net)) {
                            for (Node descendant : net.getChildren(disposableNode)) {
                                disposableNodes.push(descendant);
                            }
                            if (disposableNode.getNodeType() != NodeType.UTILITY
                                    || net.getNodes(NodeType.UTILITY).size() > 1) {
                                net.removeNode(disposableNode);
                            } else {
                                replaceUtilityPotentialWithZero(net, disposableNode);
                            }
                        }
                    }
                    changed = true;
                    break; // restart iteration since the node list changed
                }
            }
        }
    }

    private static boolean hasAllZeroProbability(Node node) {
    ...
    public static void replaceUtilityPotentialWithZero(ProbNet dan, Node node) {
        ...
        TablePotential newUtilityPotential = new TablePotential(Arrays.asList(node.getVariable()), PotentialRole.UNSPECIFIED);
    ...
    private static void updatePotentialsWithNewVariable(ProbNet instantiatedNet, Variable oldVariable,
                                                        Variable newVariable) throws NonProjectablePotentialException, IncompatibleEvidenceException.EvidenceIsIncompatibleWithOther {
        ...
        if (oldPotentials != null && !oldPotentials.isEmpty()) {
            for (Potential auxPotential : instantiatedNet.getPotentials(oldVariable)) {
                ...
                for (State auxState : newVariable.getStates()) {
                    ...
                    Potential auxPotBranch = (
                            auxPotential instanceof TablePotential || auxPotential instanceof ExactDistrPotential
                    ) ? auxPotential.project(auxEvidence) : auxPotential.tableProject(auxEvidence, null);
     ...
     */
    public static List<ProbNet> instantiate(ProbNet originalDAN, Variable variable) throws IncompatibleEvidenceException.EvidenceIsIncompatibleWithOther, NonProjectablePotentialException {
    ...
    private static void projectPotentials(ProbNet probNet, Variable variable, State state)
        ...
        if (nodePotentials != null && !nodePotentials.isEmpty()) {
                    ...
                    if (potentialProjectedFromTreeADD != null) {
                        ...
                        if (((newConditionedVariable == null) || (newConditionedVariable.getNodeType() != utilityType)) && (
                            ...
                            newPotential = new ExactDistrPotential(newVarsPotential, PotentialRole.UNSPECIFIED,
                                                                   potentialProjectedFromTreeADD.getValues());
                        } else {
    ...
    static EvidenceCase extendEvidenceCase(EvidenceCase evidenceCase, Variable x, State state)
        ...
        EvidenceCase newEvi = new EvidenceCase(evidenceCase);
...
/*	public static TablePotential firstUtilityPotential(ProbNet dan,List<TablePotential> potentials) {
    ...
    /**
     * @param dan the dan
     * @param conditioningVariablesList the conditioning variables list
    ...
    public static List<Variable> getVariablesObservedFromTheBegginning(ProbNet dan, List<Variable> conditioningVariablesList,
        ...
        if (evidenceCase != null) {
            for (Variable variable : evidenceCase.getVariables()) {
                Variable variableInDAN = dan.getVariable(variable.getName());
    ...
    public static List<Variable> getVariablesObservedByFirstDecision(ProbNet dan) {
        ...
        Deque<Variable> decisions = BasicOperations.getSequenceOfDecisions(idCopy);
        Node decisionNode = dan.getNode(decisions.peekLast());
        // Get nodes of the decision parents
        return dan.getParents(decisionNode).stream()
     ...
     * @throws IncompatibleEvidenceException if the evidence is incompatible with the network
     */
    static EvidenceCase translateEvidenceTo(ProbNet network, EvidenceCase evidenceCase) throws IncompatibleEvidenceException.FindingVariableIsMissingAState, IncompatibleEvidenceException.EvidenceIsIncompatibleWithOther {
        if (evidenceCase == null) return null;
        EvidenceCase newEvidenceCase = new EvidenceCase();
        for (Finding finding : evidenceCase.getFindings()) {
            Variable newVariable = network.getVariable(finding.getVariable().getName());
            //newVariable = network.getVariable(finding.getVariable().getName());
            if (newVariable != null) {
                State state = newVariable.getState(finding.getState());
                if (state == null) {
                    throw new IncompatibleEvidenceException.FindingVariableIsMissingAState(finding, newVariable, finding.getState());
                }
