...
public class DANDecompositionIntoSymmetricDANsInference extends DANInference {
    ...
    public DANDecompositionIntoSymmetricDANsInference(ProbNet probNet, boolean isCEA) throws NotEvaluableNetworkException.NotApplicableNetwork, NotEvaluableNetworkException.UnsatisfiedConstraints, NonProjectablePotentialException, IncompatibleEvidenceException, PotentialOperationException.DifferentSizesInPotentialsAndStates {
    ...
    public DANDecompositionIntoSymmetricDANsInference(ProbNet dan, List<Variable> conditioningVariablesList,
                                                      EvidenceCase evidenceCase, boolean isCEA) throws NotEvaluableNetworkException.NotApplicableNetwork, NotEvaluableNetworkException.UnsatisfiedConstraints, IncompatibleEvidenceException, NonProjectablePotentialException, PotentialOperationException.DifferentSizesInPotentialsAndStates {
        super(dan, isCEA);
        ...
        List<Variable> symmetricObservedVariables = new ArrayList<>(alwaysObservedVariables);
        ...
        if (DANOperations.isSymmetric(dan, evidenceCase)) {//The DAN is symmetric
            ...
            if (!asymmetricObservedVariables
                ...
                for (State state : x.getStates()) {
                    ...
                    childEvaluationDecompositionIntoSymmetricDANs(dan_x, newConditioningVariablesList, DANOperations.extendEvidenceCase(evidenceCase, x, state));
                }
                ...
                if (nextDecisions.size() == 1) { // If exactly one decision D can be made first
                    ...
                    for (var decision : nextDecisions) {
                        prioritizeDANAndChildEvaluationDecompositionIntoSymmetricDANs(dan, evidenceCase, newConditioningVariablesList, decision);
                    }
                }
     ...
     * @param conditioningVariables the conditioning variables
    ...
    private void childEvaluationDecompositionIntoSymmetricDANs(ProbNet dan, List<Variable> conditioningVariables,
                                                               EvidenceCase evidenceCase) throws IncompatibleEvidenceException, NonProjectablePotentialException, PotentialOperationException.DifferentSizesInPotentialsAndStates, NotEvaluableNetworkException.NotApplicableNetwork, NotEvaluableNetworkException.UnsatisfiedConstraints {
        addResultsOfChildEvaluation(new DANDecompositionIntoSymmetricDANsInference(dan, conditioningVariables, evidenceCase, isCEAnalysis));
