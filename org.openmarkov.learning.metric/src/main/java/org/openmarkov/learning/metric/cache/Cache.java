...
package org.openmarkov.learning.metric.cache;
...
public class Cache {
	...
	public void flush(ProbNet newNet) {
		...
		int i = 0;
        for (Variable variable : newNet.getVariables()) {
            orderedVariables.put(variable, i);
			i++;
		}
	}
	...
	public void cacheScore(PNEdit edit, double fValue) {
        Variable tail = ((BaseLinkEdit) edit).getVariableFrom();
        Variable head = ((BaseLinkEdit) edit).getVariableTo();
