 ...
 */
@MetricType(name = "MDL") public class MDLMetric extends EntropyMetric {
	 ...
	 */
	@Override protected double score(AddLinkEdit edit, boolean change) {
        
        Node destinationNode = probNet.getNode(edit.getVariableTo());
        ...
        double newNodeDimension = (edit.getVariableFrom().getNumStates() * lastNodeDimension);
		 ...
		 * calculations */
		if (change) {
			nodesDimensions.put(destinationNode.getName(), newNodeDimension);
		}
     ...
     * @return {@code double} score of the net with the given edition
	 */
	@Override protected double score(RemoveLinkEdit edit, boolean change) {
		 ...
		 * of the destination node and sum the new dimension of this node*/
        double newNodeDimension = lastNodeDimension / (edit.getVariableFrom().getNumStates());
		/*If change is true it's because we have to update the probNet values*/
		if (change) {
			nodesDimensions.put(destinationNode.getName(),newNodeDimension);
		}
	 ...
	 *
     * @param edit   {@code InvertLinkEdit}
	...
	@Override protected double score(InvertLinkEdit edit, boolean change) {
        ...
        Node initialDestinationNode = probNet.getNode(edit.getVariableTo());
        Node initialOriginNode = probNet.getNode(edit.getVariableFrom());
		double coefficient = Math.log(caseDatabase.getCases().length) / 2;
		...
		/*If change is true it's because we have to update the probNet values*/
		if (change) {
			nodesDimensions.put(initialDestinationNode.getName(), newNodeDimension);
		}
		 ...
		 * calculations */
		if (change) {
			nodesDimensions.put(initialOriginNode.getName(), newNodeDimension);
		}
	 ...
	 * of its parents.
	 *
     * @param node   {@code Node} whose dimension we want to calculate.
     * @param change {@code boolean} indicates wheter the edition is
	 *               definitive (UndoableEditHappend called this method) or not.
	...
	protected double nodeDimension(Node node, boolean change) {
        ...
        double nodeDimension = (numStates - 1) * parentsConfigurations;
		/* Store the dimension of the node to avoid repeating the calculations */
		if (change) {
			nodesDimensions.put(node.getName(), nodeDimension);
		}
