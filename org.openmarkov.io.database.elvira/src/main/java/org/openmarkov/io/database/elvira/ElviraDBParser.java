...
@SuppressWarnings("ALL") public class ElviraDBParser extends antlr.LLkParser implements ElviraDBLexerTokenTypes {

	public static final String[] _tokenNames = { "<0>", "EOF", "<2>", "NULL_TREE_LOOKAHEAD", "QUOTATION", "DATABASE",
			"WHITE", "REMARK", "COMMENT", "SEPARATOR", "IDENT", "STRING", "TRUE", "FALSE", "POINT", "LEFTP", "RIGHTP",
			"LEFTC", "RIGHTC", "LEFTB", "RIGHTB", "COMMA", "HYPHEN", "UNDERLINING", "ASSIGNMENT", "MEMORY",
			"VISUALPRECISION", "VERSION", "DEFAULT", "NODE", "KIND", "OF", "CHANCE", "DECISION", "UTILITY", "TYPE",
			"VARIABLE", "RELATION", "FINITE", "STATES", "POSX", "POSY", "CONTINUOUS", "RELEVANCE", "PURPOSE", "MIN",
			"MAX", "NUM", "PRECISION", "TITLE", "AUTHOR", "WHOCHANGED", "WHENCHANGED", "KINDOFGRAPH", "NUMBER",
			"CASES" };
	 ...
	 * Messages file
	 */
	private Logger logFile = LogManager.getLogger(ElviraDBParser.class);
	...
	/**
	 * Adds a <code>Node</code> to the <code>ProbNet</code>.
	 */
	public Node addNode(HashMap<String, Object> infoNode, State[] states, NodeType nodeType, String nodeName) {
		...
		Node node = null;
		try {
			node = probNet.addNode(fsVariable, nodeType);
			...
			node.setAdditionalProperties(properties);
		} catch (Exception e) {
	...
	public final HashMap<String, Object> database() throws RecognitionException, TokenStreamException {
		...
		String titleNet = null, commentNet = null, authorNet = null, whochangedNet = null, whenchangedNet = null, name = null, kindOfGraph = null;
	...
	public final void relation() throws RecognitionException, TokenStreamException {
		...
		try {      // for error handling
			...
			Node node = probNet.getNode(theVariables[i]);
			Variable fsVariable = node.getVariable();
			...
			// 3. Reorder the potential in the OpenMarkov mode
			tablePotential = IOOperations.elvira2OpenMarkovPotential(tablePotential);
			...
			probNet.addPotential(tablePotential);
				...
				e.printStackTrace();
				    if (messagesImportance >= logFile.importanceThreshold) {
				    	logFile.debug("Problems translating elvira potentials to OpenMarkov "
				    	    + "potentials");
				    }
			}
	...
	public final void node() throws RecognitionException, TokenStreamException {
		...
		try {      // for error handling
			if ((LA(1) == NODE) && (LA(2) == IDENT)) {
				...
				Node node = addNode(infoNode, states, (NodeType) infoNode.get("NodeType"), id.getText());
				infoNode.put("ProbNode", node);
				...
				if (infoNode != null) {
					...
					if (infoNode.get("NodeStates") != null) {
						logFile.debug(". Num states: " + ((String[]) infoNode.get("NodeStates")).length);
					}
				} else {
				...
				Node node = addNode(infoNode, states, (NodeType) infoNode.get("NodeType"), id3.getText());
				infoNode.put("ProbNode", node);
				...
				Node node = addNode(infoNode, states, (NodeType) infoNode.get("NodeType"), id2.getText());
				infoNode.put("ProbNode", node);
				...
				Node node = addNode(infoNode, states, (NodeType) infoNode.get("NodeType"), id4.getText());
				infoNode.put("ProbNode", node);
	...
	public final HashMap<String, Object> bodynode() throws RecognitionException, TokenStreamException {
		...
		double relevanceNode = Double.MIN_VALUE, minNode = Double.POSITIVE_INFINITY, maxNode = Double.NEGATIVE_INFINITY;
		...
		try {      // for error handling
			...
			if (nodeType != NodeType.UTILITY) {
				if (statesVar == null) {
					...
					infoNode.put("UseDefaultStates", Boolean.TRUE);
				} else {
					infoNode.put("NodeStates", statesVar);
					infoNode.put("UseDefaultStates", Boolean.FALSE);
				}
			}
	...
	public final NodeType typenode() throws RecognitionException, TokenStreamException {
		...
		try {      // for error handling
			if ((LA(1) == KIND) && (LA(2) == HYPHEN) && (LA(3) == OF) && (LA(4) == HYPHEN) && (LA(5) == NODE) && (
					LA(6) == ASSIGNMENT
			) && (LA(7) == CHANCE)) {
				typeofnode();
				astFactory.addASTChild(currentAST, returnAST);
			...
			} else if ((LA(1) == KIND) && (LA(2) == HYPHEN) && (LA(3) == OF) && (LA(4) == HYPHEN) && (LA(5) == NODE)
					&& (LA(6) == ASSIGNMENT) && (LA(7) == DECISION)) {
				typeofnode();
				astFactory.addASTChild(currentAST, returnAST);
				a2 = LT(1);
	...
	public final VariableType typevariable() throws RecognitionException, TokenStreamException {
		...
		try {      // for error handling
			if ((LA(1) == TYPE) && (LA(2) == HYPHEN) && (LA(3) == OF) && (LA(4) == HYPHEN) && (LA(5) == VARIABLE) && (
					LA(6) == ASSIGNMENT
			) && (LA(7) == FINITE)) {
				typeofv();
				astFactory.addASTChild(currentAST, returnAST);
			...
			} else if ((LA(1) == TYPE) && (LA(2) == HYPHEN) && (LA(3) == OF) && (LA(4) == HYPHEN) && (LA(5) == VARIABLE)
					&& (LA(6) == ASSIGNMENT) && (LA(7) == CONTINUOUS)) {
				typeofv();
				astFactory.addASTChild(currentAST, returnAST);
				a2 = LT(1);
				a2_AST = astFactory.create(a2);
